...
internal abstract class PythonCodeTemplate
{
    ...
    public static string FormatStringValue(string? value)
    {
        ...
        if (value.Contains('\n') || value.Contains('\r'))
        {
            ...
            return $"\"\"\"{Environment.NewLine}{escapedValue}{Environment.NewLine}\"\"\"";
        }

        // For strings with quotes or backslashes, escape them
    ...
    public static string FormatDataValue(DataValue value) =>
        value switch
        {
            ...
            DateDataValue dateValue => $"datetime({dateValue.Value.Year}, {dateValue.Value.Month}, {dateValue.Value.Day})",
            DateTimeDataValue datetimeValue => $"datetime.fromtimestamp({datetimeValue.Value.ToUnixTimeSeconds()})",
            TimeDataValue timeValue => $"timedelta(seconds={timeValue.Value.TotalSeconds})",
    ...
    public static TTarget FormatEnum<TSource, TTarget>(TSource value, IDictionary<TSource, TTarget> map, TTarget? defaultValue = default)
    {
        ...
        {
            throw new DeclarativeModelException($"No default value supplied for '{typeof(TTarget).Name}'");
        }
    ...
    /// <summary>
    /// Convert a C# style name to Python snake_case
    /// </summary>
    public static string ToPythonName(string name)
    {
        ...
        for (int i = 0; i < name.Length; i++)
        {
            ...
            if (char.IsUpper(c))
            {
                ...
                result.Append(char.ToLower(c, System.Globalization.CultureInfo.InvariantCulture));
            }
            else
            {
                result.Append(c);
