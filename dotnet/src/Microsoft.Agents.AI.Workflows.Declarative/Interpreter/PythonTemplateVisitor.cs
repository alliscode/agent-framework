...
internal sealed class PythonTemplateVisitor : DialogActionVisitor
{
    private readonly string _rootId;
    private readonly WorkflowModel<string> _workflowModel;

    public PythonTemplateVisitor(
        string workflowId,
        WorkflowTypeInfo typeInfo)
    {
        this._rootId = workflowId;
        this._workflowModel = new WorkflowModel<string>(new PythonRootTemplate(workflowId, typeInfo));
    ...
    {
        PythonCodeBuilder builder = new(this._rootId);
    ...
    protected override void Visit(ActionScope item)
    {
        ...
        this.ContinueWith(new PythonEmptyTemplate(item.Id.Value, this._rootId), parentId, condition: null, CompletionHandler);
    ...
    public override void VisitConditionItem(ConditionItem item)
    {
        ...
        string parentId = GetParentId(item);
        PythonConditionGroupTemplate? conditionGroup = this._workflowModel.LocateParent<PythonConditionGroupTemplate>(parentId);
        if (conditionGroup is not null)
        {
            string stepId = ConditionGroupExecutor.Steps.Item(conditionGroup.Model, item);
            this._workflowModel.AddNode(new PythonEmptyTemplate(stepId, this._rootId), parentId, CompletionHandler);
    ...
    protected override void Visit(ConditionGroup item)
    {
        ...
        PythonConditionGroupTemplate action = new(item);
        ...
        foreach (ConditionItem conditionItem in item.Conditions)
        {
            ...
            this._workflowModel.AddLink(action.Id, lastConditionItemId, $@"ActionExecutor.is_match(""{lastConditionItemId}"", result)");
        ...
        if (item.ElseActions?.Actions.Length > 0)
        {
            ...
            this._workflowModel.AddLink(action.Id, stepId, $@"ActionExecutor.is_match(""{stepId}"", result)");
        }
    }
    ...
    protected override void Visit(GotoAction item)
    {
        ...
        PythonDefaultTemplate action = new(item, this._rootId);
        this.ContinueWith(action);
    ...
    protected override void Visit(Foreach item)
    {
        ...
        PythonForeachTemplate action = new(item);
        ...
        this.ContinueWith(new PythonEmptyTemplate(loopId, this._rootId, $"{action.Id.FormatName()}.take_next_async"), action.Id);
        ...
        this._workflowModel.AddLink(loopId, continuationId, $"not {action.Id.FormatName()}.has_value");

        // Transition to start of inner actions if there is a current item
        string startId = ForeachExecutor.Steps.Start(action.Id);
        this._workflowModel.AddNode(new PythonEmptyTemplate(startId, this._rootId), action.Id);
        this._workflowModel.AddLink(loopId, startId, $"{action.Id.FormatName()}.has_value");

        void CompletionHandler()
        {
            // Transition to end of inner actions
            string endActionsId = ForeachExecutor.Steps.End(action.Id); // Loop continuation
            this.ContinueWith(new PythonEmptyTemplate(endActionsId, this._rootId, $"{action.Id.FormatName()}.reset_async"), action.Id);
            // Transition to select the next item
    ...
    protected override void Visit(BreakLoop item)
    {
        ...
        PythonForeachTemplate? loopAction = this._workflowModel.LocateParent<PythonForeachTemplate>(item.GetParentId());
    ...
    protected override void Visit(CreateConversation item)
    {
        ...
        this.ContinueWith(new PythonCreateConversationTemplate(item));
    }
    ...
    protected override void Visit(AddConversationMessage item)
    {
        ...
        this.ContinueWith(new PythonAddConversationMessageTemplate(item));
    }
    ...
    protected override void Visit(CopyConversationMessages item)
    {
        ...
        this.ContinueWith(new PythonCopyConversationMessagesTemplate(item));
    }
    ...
    protected override void Visit(InvokeAzureAgent item)
    {
        ...
        this.ContinueWith(new PythonInvokeAzureAgentTemplate(item));
    }
    ...
    protected override void Visit(RetrieveConversationMessage item)
    {
        ...
        this.ContinueWith(new PythonRetrieveConversationMessageTemplate(item));
    }
    ...
    protected override void Visit(RetrieveConversationMessages item)
    {
        ...
        this.ContinueWith(new PythonRetrieveConversationMessagesTemplate(item));
    }
    ...
    protected override void Visit(SetVariable item)
    {
        ...
        this.ContinueWith(new PythonSetVariableTemplate(item));
    }
    ...
    protected override void Visit(SetMultipleVariables item)
    {
        ...
        this.ContinueWith(new PythonSetMultipleVariablesTemplate(item));
    }
    ...
    protected override void Visit(SetTextVariable item)
    {
        ...
        this.ContinueWith(new PythonSetTextVariableTemplate(item));
    }
    ...
    protected override void Visit(ClearAllVariables item)
    {
        ...
        this.ContinueWith(new PythonClearAllVariablesTemplate(item));
    }
    ...
    protected override void Visit(ResetVariable item)
    {
        ...
        this.ContinueWith(new PythonResetVariableTemplate(item));
    }
    ...
    protected override void Visit(ParseValue item)
    {
        ...
        this.ContinueWith(new PythonParseValueTemplate(item));
    }
    ...
    protected override void Visit(SendActivity item)
    {
        ...
        this.ContinueWith(new PythonSendActivityTemplate(item));
    }
    ...
    private string ContinuationFor(string actionId, string parentId, string? stepAction = null)
    {
        ...
        this._workflowModel.AddNode(new PythonEmptyTemplate(actionId, this._rootId, stepAction), parentId);

        return actionId;
    }

    private void RestartAfter(string actionId, string parentId) =>
        this._workflowModel.AddNode(new PythonEmptyTemplate(WorkflowActionVisitor.Steps.Restart(actionId), this._rootId), parentId);
