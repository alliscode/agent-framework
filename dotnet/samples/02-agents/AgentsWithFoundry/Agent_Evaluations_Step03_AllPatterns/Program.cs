...
// Create test agent using the Responses API (no server-side agent lifecycle)
...
var multiTurnConversation = new List<ChatMessage>
{
    ...
    new(ChatRole.Assistant, "Seattle is 62F, cloudy with a chance of rain."),
    new(ChatRole.User, "And Paris?"),
    new(ChatRole.Assistant, "Paris is 68F, partly sunny."),
    new(ChatRole.User, "Compare them."),
    new(ChatRole.Assistant, "Seattle is cooler at 62F with rain likely, while Paris is warmer at 68F and sunnier."),
};

// Strategy 1: LAST_TURN (default) -- evaluates the final response
var lastTurnItem = new EvalItem(
    "Compare them.",
    "Seattle is cooler at 62F with rain likely, while Paris is warmer at 68F and sunnier.",
...
Console.WriteLine("All evaluation patterns completed.");
...
{
    string status = results.AllPassed ? "PASS ALL" : "SOME FAILED";
    ...
    if (results.SubResults is not null)
    {
        ...
        {
            string subStatus = sub.AllPassed ? "PASS" : "FAIL";
